...
fun TripNextApp(viewModel: AppViewModel, startWithQuickExpense: Boolean, visualTheme: TripVisualTheme, onVisualThemeChange: (TripVisualTheme) -> Unit) {
    ...
    Scaffold(
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            if (copilot) AiPlannerScreen(state, viewModel.aiPlan, viewModel.aiProposal, viewModel.aiProposalHistory, viewModel.aiLoading, viewModel.aiError, viewModel.session != null, viewModel.selectedAiItemIds, onClose = { copilot = false }, onGenerate = viewModel::generateAiPlan, onImport = viewModel::importAiProposal, onToggleItem = viewModel::toggleAiItem, onSelectAll = viewModel::selectAllAiItems, onSaveProfile = viewModel::saveTravelProfile, onLoad = viewModel::loadAiProposals, onDismissProposal = viewModel::dismissAiProposal)
...
@Composable private fun AiPlannerScreen(state: AppUiState, result: String, proposal: AiTravelProposal?, history: List<AiProposalHistory>, loading: Boolean, error: String?, sessionAvailable: Boolean, selectedItemIds: Set<String>, onClose: () -> Unit, onGenerate: () -> Unit, onImport: () -> Unit, onToggleItem: (String) -> Unit, onSelectAll: (Boolean) -> Unit, onSaveProfile: (TravelProfileInput) -> Unit, onLoad: () -> Unit, onDismissProposal: () -> Unit) {
    var showProfile by remember { mutableStateOf(false) }
    LaunchedEffect(state.activeTrip?.id, sessionAvailable) { if (sessionAvailable) onLoad() }
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        state.activeTrip?.let { trip -> item { Surface(shape = RoundedCornerShape(16.dp), color = MaterialTheme.colorScheme.surface, border = BorderStroke(1.dp, PrototypeBorder)) { Row(Modifier.fillMaxWidth().clickable { showProfile = true }.padding(14.dp), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Default.Tune, null, tint = MaterialTheme.colorScheme.primary); Column(Modifier.weight(1f).padding(horizontal = 11.dp)) { Text("Perfil de planejamento", fontWeight = FontWeight.Medium); Text(listOf(trip.origin.ifBlank { "Origem pendente" }, when (trip.pace) { "LIGHT" -> "Ritmo leve"; "INTENSE" -> "Ritmo intenso"; else -> "Ritmo equilibrado" }, trip.interests.ifBlank { "Adicione interesses" }).joinToString(" · "), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }; Icon(Icons.Default.ChevronRight, null) } } } }
        ...
        proposal?.let { plan ->
            item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { Text("Disponível até ${formatProposalDate(plan.expiresAt)}", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.onSurfaceVariant); TextButton(onDismissProposal, enabled = !loading) { Icon(Icons.Default.DeleteOutline, null, Modifier.size(17.dp)); Spacer(Modifier.width(5.dp)); Text("Descartar") } } }
            ...
            item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { SectionTitle("Revise cada mudança"); TextButton({ onSelectAll(selectedItemIds.size != plan.selectableIds.size) }) { Text(if (selectedItemIds.size == plan.selectableIds.size) "Desmarcar tudo" else "Selecionar tudo") } } }
            if (plan.itinerary.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(7.dp)) { plan.itinerary.forEach { suggestion -> val ignored = suggestion.action in setOf("SKIP_DUPLICATE", "SKIP_UNCHANGED"); Surface(shape = RoundedCornerShape(13.dp), color = MaterialTheme.colorScheme.surface) { Row(Modifier.fillMaxWidth().padding(8.dp), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }, enabled = !ignored); Column(Modifier.weight(1f).padding(start = 4.dp)) { Text("${aiActionLabel(suggestion.action)} · Dia ${suggestion.dayOffset + 1} · ${suggestion.time} · ${suggestion.title}", fontWeight = FontWeight.Medium); Text(if (ignored) "Sem mudança necessária · ignorado" else listOf(suggestion.location, suggestion.reason, money(suggestion.estimatedCostMinor)).filter(String::isNotBlank).joinToString(" · "), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant); suggestion.changes.forEach { change -> Text("${change.field}: ${change.before.ifBlank { "—" }} → ${change.after.ifBlank { "—" }}", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.primary) } } } } } } }
            if (plan.checklist.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) { SectionTitle("Checklist"); plan.checklist.forEach { suggestion -> val ignored = suggestion.action in setOf("SKIP_DUPLICATE", "SKIP_UNCHANGED"); Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }, enabled = !ignored); Column { Text("${aiActionLabel(suggestion.action)} · ${suggestion.name}", style = MaterialTheme.typography.bodySmall); suggestion.changes.forEach { change -> Text("${change.field}: ${change.before.ifBlank { "—" }} → ${change.after.ifBlank { "—" }}", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.primary) } } } } } }
            if (plan.budgets.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) { SectionTitle("Orçamento previsto"); plan.budgets.forEach { suggestion -> val ignored = suggestion.action == "SKIP_UNCHANGED"; Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }, enabled = !ignored); Column { Text("${aiActionLabel(suggestion.action)} · ${suggestion.category} · ${suggestion.percent}% · ${suggestion.reason}", style = MaterialTheme.typography.bodySmall); suggestion.changes.forEach { change -> Text("${change.before}% → ${change.after}%", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.primary) } } } } } }
            item { Button(onClick = onImport, enabled = selectedItemIds.isNotEmpty() && !loading, modifier = Modifier.fillMaxWidth().height(52.dp)) { Icon(Icons.Default.Download, null); Spacer(Modifier.width(8.dp)); Text("Adicionar ${selectedItemIds.size} item(ns)") } }
        ...
        if (history.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) { SectionTitle("Histórico de propostas"); history.forEach { item -> Surface(shape = RoundedCornerShape(12.dp), color = MaterialTheme.colorScheme.surface) { Row(Modifier.fillMaxWidth().padding(11.dp), horizontalArrangement = Arrangement.SpaceBetween) { Text(formatProposalDate(item.createdAt), style = MaterialTheme.typography.bodySmall); Text(when (item.status) { "DRAFT" -> "Pendente"; "APPLIED" -> "Aplicada"; "DISMISSED" -> "Descartada"; "EXPIRED" -> "Expirada"; else -> item.status }, style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.primary) } } } } }
    ...
    if (showProfile) state.activeTrip?.let { trip -> TravelProfileSheet(trip, { showProfile = false }) { onSaveProfile(it); showProfile = false } }
}

@OptIn(ExperimentalMaterial3Api::class)
@Composable private fun TravelProfileSheet(trip: TripEntity, onDismiss: () -> Unit, onSave: (TravelProfileInput) -> Unit) {
    var origin by remember { mutableStateOf(trip.origin) }; var flexible by remember { mutableStateOf(trip.flexibleDates) }; var children by remember { mutableStateOf(trip.children.toString()) }; var ages by remember { mutableStateOf(trip.childAges) }; var interests by remember { mutableStateOf(trip.interests) }; var avoid by remember { mutableStateOf(trip.avoidPreferences) }; var pace by remember { mutableStateOf(trip.pace) }; var startHour by remember { mutableStateOf(trip.preferredStartHour.toString()) }; var rest by remember { mutableStateOf(trip.restMinutes.toString()) }; var food by remember { mutableStateOf(trip.foodPreferences) }; var dietary by remember { mutableStateOf(trip.dietaryRestrictions) }; var transport by remember { mutableStateOf(trip.preferredTransport) }; var walking by remember { mutableStateOf(trip.maxWalkingMinutes.toString()) }; var mobility by remember { mutableStateOf(trip.mobilityNeeds) }
    ModalBottomSheet(onDismissRequest = onDismiss, containerColor = MaterialTheme.colorScheme.surface) {
        LazyColumn(Modifier.fillMaxWidth(), contentPadding = PaddingValues(horizontal = 18.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(11.dp)) {
            item { Text("Como vocês gostam de viajar?", style = MaterialTheme.typography.titleLarge); Text("Este perfil acompanha somente esta viagem e orienta todas as propostas do Copiloto.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
            item { OutlinedTextField(origin, { origin = it }, Modifier.fillMaxWidth(), label = { Text("Cidade/aeroporto de origem") }, singleLine = true) }
            item { Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Text("Datas flexíveis", Modifier.weight(1f)); Switch(flexible, { flexible = it }) } }
            item { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(children, { children = it.filter(Char::isDigit) }, Modifier.weight(1f), label = { Text("Crianças") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true); OutlinedTextField(ages, { ages = it }, Modifier.weight(2f), label = { Text("Idades") }, placeholder = { Text("5, 9") }, singleLine = true) } }
            item { Text("Ritmo", style = MaterialTheme.typography.labelLarge); Row(horizontalArrangement = Arrangement.spacedBy(7.dp)) { listOf("LIGHT" to "Leve", "BALANCED" to "Equilibrado", "INTENSE" to "Intenso").forEach { (value, label) -> FilterChip(pace == value, { pace = value }, { Text(label) }) } } }
            item { OutlinedTextField(interests, { interests = it }, Modifier.fillMaxWidth(), label = { Text("Interesses") }, placeholder = { Text("arte, praias, gastronomia, natureza") }) }
            item { OutlinedTextField(avoid, { avoid = it }, Modifier.fillMaxWidth(), label = { Text("O que evitar") }, placeholder = { Text("filas, vida noturna, lugares lotados") }) }
            item { Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { OutlinedTextField(startHour, { startHour = it.filter(Char::isDigit) }, Modifier.weight(1f), label = { Text("Começar às") }, suffix = { Text("h") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true); OutlinedTextField(rest, { rest = it.filter(Char::isDigit) }, Modifier.weight(1f), label = { Text("Descanso") }, suffix = { Text("min") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true); OutlinedTextField(walking, { walking = it.filter(Char::isDigit) }, Modifier.weight(1f), label = { Text("Caminhada") }, suffix = { Text("min") }, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Number), singleLine = true) } }
            item { OutlinedTextField(food, { food = it }, Modifier.fillMaxWidth(), label = { Text("Estilo de alimentação") }, placeholder = { Text("local, econômica, restaurantes especiais") }) }
            item { OutlinedTextField(dietary, { dietary = it }, Modifier.fillMaxWidth(), label = { Text("Restrições alimentares") }) }
            item { OutlinedTextField(transport, { transport = it }, Modifier.fillMaxWidth(), label = { Text("Transporte preferido") }, placeholder = { Text("metrô, trem, carro, bicicleta") }) }
            item { OutlinedTextField(mobility, { mobility = it }, Modifier.fillMaxWidth(), label = { Text("Mobilidade e acessibilidade") }, placeholder = { Text("cadeira de rodas, evitar escadas, pausas") }) }
            item { Button({ onSave(TravelProfileInput(origin, flexible, children.toIntOrNull() ?: 0, ages, interests, avoid, pace, startHour.toIntOrNull() ?: 9, rest.toIntOrNull() ?: 60, food, dietary, transport, walking.toIntOrNull() ?: 30, mobility)) }, Modifier.fillMaxWidth().padding(bottom = 24.dp).height(52.dp)) { Text("Salvar perfil da viagem") } }
        }
    }
}
...
private fun aiActionLabel(action: String) = when (action) { "ADD" -> "Adicionar"; "UPDATE" -> "Alterar"; "MOVE" -> "Mover"; "REMOVE" -> "Remover"; "SKIP_DUPLICATE" -> "Duplicado"; else -> "Sem alteração" }
private fun formatProposalDate(value: String): String = runCatching { java.time.Instant.parse(value).atZone(ZoneId.systemDefault()).format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")) }.getOrDefault(value)
