...
fun TripNextApp(viewModel: AppViewModel, startWithQuickExpense: Boolean, visualTheme: TripVisualTheme, onVisualThemeChange: (TripVisualTheme) -> Unit) {
    ...
    Scaffold(
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            if (copilot) AiPlannerScreen(state, viewModel.aiPlan, viewModel.aiProposal, viewModel.aiLoading, viewModel.aiError, onClose = { copilot = false }, onGenerate = viewModel::generateAiPlan, onImport = viewModel::importAiProposal)
                ...
                AppTab.SETTINGS -> SettingsScreen(hideValues, { hideValues = it }, visualTheme, onVisualThemeChange)
            }
        }
    }
...
@Composable private fun AiPlannerScreen(state: AppUiState, result: String, proposal: AiTravelProposal?, loading: Boolean, error: String?, onClose: () -> Unit, onGenerate: (String) -> Unit, onImport: () -> Unit) {
    val context = LocalContext.current
    val preferences = remember { context.getSharedPreferences("tripnext_ai", 0) }
    var apiKey by remember { mutableStateOf(preferences.getString("gemini_key", "").orEmpty()) }
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (editingKey) item {
            Surface(shape = RoundedCornerShape(16.dp), color = MaterialTheme.colorScheme.surface, border = BorderStroke(1.dp, PrototypeBorder)) { Column(Modifier.padding(14.dp)) { Text("Conectar Gemini", style = MaterialTheme.typography.bodyLarge, fontWeight = FontWeight.Medium); Text("Cole uma chave criada no Google AI Studio. Ela fica somente neste aparelho.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(vertical = 7.dp)); OutlinedTextField(apiKey, { apiKey = it.trim() }, Modifier.fillMaxWidth(), label = { Text("Chave da API") }, visualTransformation = PasswordVisualTransformation(), singleLine = true); Button(onClick = { preferences.edit().putString("gemini_key", apiKey).apply(); editingKey = false; onGenerate(apiKey) }, enabled = apiKey.isNotBlank(), modifier = Modifier.fillMaxWidth().padding(top = 10.dp)) { Icon(Icons.Default.Link, null); Spacer(Modifier.width(7.dp)); Text("Conectar e gerar plano") } } }
        } else item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { Button(onClick = { onGenerate(apiKey) }, enabled = !loading, modifier = Modifier.weight(1f)) { if (loading) CircularProgressIndicator(Modifier.size(18.dp), strokeWidth = 2.dp) else Icon(Icons.Default.AutoAwesome, null, Modifier.size(18.dp)); Spacer(Modifier.width(7.dp)); Text(if (result.isBlank()) "Gerar plano" else "Atualizar plano") }; OutlinedIconButton(onClick = { editingKey = true }) { Icon(Icons.Default.Key, "Alterar chave") } }
        }
        ...
        proposal?.let { plan ->
            ...
            item { Button(onClick = onImport, Modifier.fillMaxWidth().height(52.dp)) { Icon(Icons.Default.Download, null); Spacer(Modifier.width(8.dp)); Text("Adicionar ao meu planejamento") } }
            if (plan.sources.isNotEmpty()) item { Text("Fontes: ${plan.sources.joinToString(" · ")}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) }
        }
...
@Composable private fun SettingsScreen(hideValues: Boolean, onHideValuesChange: (Boolean) -> Unit, visualTheme: TripVisualTheme, onVisualThemeChange: (TripVisualTheme) -> Unit) {
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(start = 16.dp, top = 16.dp, end = 16.dp, bottom = 110.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        item { Surface(shape = RoundedCornerShape(22.dp), color = MaterialTheme.colorScheme.surface) { Row(Modifier.fillMaxWidth().padding(18.dp), verticalAlignment = Alignment.CenterVertically) { Box(Modifier.size(54.dp).background(MaterialTheme.colorScheme.primaryContainer, CircleShape), contentAlignment = Alignment.Center) { Text("J", color = MaterialTheme.colorScheme.onPrimaryContainer, style = MaterialTheme.typography.titleLarge) }; Column(Modifier.padding(start = 14.dp)) { Text("Jefferson", style = MaterialTheme.typography.titleMedium); Text("Viagem sincronizada", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant) } } } }
        item { ModernSettingsGroup("CONTA") { ModernSettingsRow(Icons.Default.Person, "Seu perfil", "Jefferson"); HorizontalDivider(color = PrototypeBorder); ModernSettingsRow(Icons.Default.Group, "Viagem compartilhada", "3 pessoas") } }
        ...
        item { ModernSettingsGroup("DADOS") { ModernSettingsRow(Icons.Default.Sync, "Sincronização", "Offline pronto"); HorizontalDivider(color = PrototypeBorder); ModernSettingsRow(Icons.Default.Security, "Segurança", "2FA desativado") } }
    }
