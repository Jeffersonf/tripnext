...
@Composable private fun AiPlannerScreen(state: AppUiState, result: String, proposal: AiTravelProposal?, loading: Boolean, error: String?, sessionAvailable: Boolean, selectedItemIds: Set<String>, onClose: () -> Unit, onGenerate: () -> Unit, onImport: () -> Unit, onToggleItem: (String) -> Unit, onSelectAll: (Boolean) -> Unit) {
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        proposal?.let { plan ->
            ...
            if (plan.itinerary.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(7.dp)) { plan.itinerary.forEach { suggestion -> val duplicate = suggestion.action == "SKIP_DUPLICATE"; Surface(shape = RoundedCornerShape(13.dp), color = MaterialTheme.colorScheme.surface) { Row(Modifier.fillMaxWidth().padding(8.dp), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }, enabled = !duplicate); Column(Modifier.weight(1f).padding(start = 4.dp)) { Text("Dia ${suggestion.dayOffset + 1} · ${suggestion.time} · ${suggestion.title}", fontWeight = FontWeight.Medium); Text(if (duplicate) "Já existe no roteiro · ignorado" else listOf(suggestion.location, suggestion.reason, money(suggestion.estimatedCostMinor)).filter(String::isNotBlank).joinToString(" · "), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } } } } } }
            if (plan.checklist.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) { SectionTitle("Checklist"); plan.checklist.forEach { suggestion -> val duplicate = suggestion.action == "SKIP_DUPLICATE"; Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }, enabled = !duplicate); Text(if (duplicate) "${suggestion.name} · já existe" else suggestion.name, style = MaterialTheme.typography.bodySmall) } } } }
            if (plan.budgets.isNotEmpty()) item { Column(verticalArrangement = Arrangement.spacedBy(6.dp)) { SectionTitle("Orçamento previsto"); plan.budgets.forEach { suggestion -> Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) { Checkbox(suggestion.id in selectedItemIds, { onToggleItem(suggestion.id) }); Text("${suggestion.category} · ${suggestion.percent}% · ${suggestion.reason}", style = MaterialTheme.typography.bodySmall) } } } }
