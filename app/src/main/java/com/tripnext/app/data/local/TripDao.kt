...
@Dao
interface TripDao {
    @Query("SELECT * FROM trips ORDER BY isActive DESC, startDate") fun observeTrips(): Flow<List<TripEntity>>
    ...
    @Query("SELECT * FROM itinerary_events WHERE tripId = :tripId ORDER BY startsAt") fun observeItinerary(tripId: String): Flow<List<ItineraryEventEntity>>
    ...
    @Transaction suspend fun deleteTripFully(id: String) { deleteExpensesForTrip(id); deleteEventsForTrip(id); deleteChecklistForTrip(id); deleteBudgetsForTrip(id); deleteGoalsForTrip(id); deleteReservationsForTrip(id); deleteVehiclesForTrip(id); deleteParticipantsForTrip(id); deleteTripRow(id) }
