...
test("OpenAPI is valid YAML and documents every public route", async () => {
  ...
  for (const route of ["/health", "/api/auth/register", "/api/auth/login", "/api/me", "/api/trips", "/api/trips/{tripId}", "/api/sync/push", "/api/sync/pull"]) assert.ok(contract.paths[route], `${route} is missing`);
