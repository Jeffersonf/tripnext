...
export class PostgresStore {
  ...
  async pull(userId, cursor = 0, tripId = null) { const { rows } = await this.pool.query("SELECT c.* FROM sync_changes c JOIN trip_members m ON m.trip_id=c.trip_id WHERE m.user_id=$1 AND c.sequence>$2 AND ($3::uuid IS NULL OR c.trip_id=$3) ORDER BY c.sequence LIMIT 1000", [userId, Number(cursor), tripId]); const changes = rows.map(row => ({ sequence: Number(row.sequence), tripId: row.trip_id, entityType: row.entity_type, entityId: row.entity_id, version: Number(row.version), deleted: row.deleted, payload: row.payload, changedBy: row.changed_by, changedAt: row.changed_at })); return { cursor: changes.at(-1)?.sequence || Number(cursor), changes }; }
